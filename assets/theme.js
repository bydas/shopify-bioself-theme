        ...
        acceptBtn.addEventListener("click", function(e) {
          ...
          if (managePrefsBtn) {
            ...
            const prefsObserver = new MutationObserver(function(mutations, prefsObs) {
              ...
              if (prefsDialog) {
                const dialogAcceptBtn = prefsDialog.querySelector("#shopify-pc__prefs__header-accept");
                ...
                if (dialogAcceptBtn) {
                  ...
                  dialogAcceptBtn.click();
                  prefsObs.disconnect();
                }
              }
            });
            
            prefsObserver.observe(document.body, { childList: true, subtree: true });
          }
        });
      }
...
(function (factory) {
  ...
  var ProductVariants = /*#__PURE__*/function () {
    ...
    _createClass(ProductVariants, [{
      ...
      /**
       * PO automatically adds a "disabled" state to sold out/unavailable variant. When we change the variant we have to recompute
       * all the selectors
       */
  ...
  */
  ...
  var SearchBar = /*#__PURE__*/function () {
    ...
    },
    {
      ...
      value: function _addToCart(event, target) {
        ...
        fetch("".concat(window.routes.cartAddUrl, ".js"), {
          ...
          if (response.ok) {
            target.removeAttribute('disabled'); // We simply trigger an event so the mini-cart can re-render
            _this9.element.dispatchEvent(new CustomEvent('product:added', {
              bubbles: true,
              detail: {
    ...
    
    {
      ...
      /**
       * PO allows to display a filter by type. When its value change, we re-do a new search to get the new data
       */
...
document.querySelectorAll('.promotion_modal').forEach(button => {
    ...
    document.addEventListener('click', (event) => {
        if(event.target.closest('.promo_modal') == null && event.target.closest('.promotion_modal') == null) {
            promoModal.classList.remove('show-promotion-modal');
            promoModal.classList.add('hide-promotion-modal');
        }
    });
...
if (document && curUrl.includes('/blogs/blog-page')) {
  ...
  if ( curUrl.includes('?customer_posted=true') && curUrl.includes('#newsletter-blog-sidebar-0') ) {
    console.log("URL contains '?customer_posted=true' and '#newsletter-blog-sidebar-0'");
    bottom_newsletter_text.style.display = "none";

  } else if (sidebar_newsletter_text && curUrl.includes('?customer_posted=true')) {
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function checkForNetReviewsRateAndStars(mutationsList, observer) {
    productItems.forEach((productItem) => {
      const hasNetReviewsRateAndStars = productItem.querySelectorAll('.coral').length > 0;
      ...
      if (hasNetReviewsRateAndStars) {
        console.log('This div contains elements with class "netreviews_review_rate_and_stars".');
        noReviews_div.style.display = 'none';
        review_div.style.display = 'block';

      } else {
        console.log('This div does not contain elements with class "netreviews_review_rate_and_stars".');
...
document.addEventListener("DOMContentLoaded", function() {
  const submitButtons = document.querySelectorAll('.form__submit, .button.button--primary.button--full.form-submit-button');
