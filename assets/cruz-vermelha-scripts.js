  ...
  if (cruz_modal && overlay_modal) {
    ...
    cruz_form?.addEventListener("submit", async function (e) {
      ...
      fetch(
        ...
        .then((res) => {
          if (!res.ok) {
            cruz_nif_message.innerText = "Número Inválido"
        ...
        .then((data) => {
          const { status, msg, response, coupon } = data

          if (response && !response.valid) {
            cruz_nif_message.innerText = response.message ?? msg
            cruz_nif_message.style.color = "red"
            return
          }
          ...
          if (status) {
            if (data.customer_id && customer_id !== data.customer_id) {
              cruz_nif_message.innerText =
                "Número já associado a uma conta, por favor, faça login para o aplicar"
              cruz_nif_message.style.color = "red"
              return
            }
            ...
            if (coupon) {
              ...
              setTimeout(function () {
                localStorage.setItem("CV_Discount", coupon)
    ...
    escape_cruz_modal?.addEventListener("click", function () {
      ...
      overlay_modal.style.zIndex = '1002'
    })
  }
})
;(function () {
